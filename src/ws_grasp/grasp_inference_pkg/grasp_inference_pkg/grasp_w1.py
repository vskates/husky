...
def _norm_frame(s: str) -> str:
    return s.strip().lstrip("/") if s else ""
...
class GraspInferenceNode(Node):
    def __init__(self):
        ...
        S = int(self.get_parameter("hm_size").value)
        res = float(self.get_parameter("hm_resolution").value)
        pmin = np.array(self.get_parameter("plane_min").value, np.float32)
        pmax = np.array(self.get_parameter("plane_max").value, np.float32)
        ...
        # ---- tf ----
        self.tf_buffer = tf2_ros.Buffer(Duration(seconds=5))
        self.tf_listener = tf2_ros.TransformListener(self.tf_buffer, self)

        # ---- pubs ----
        self.pub_color = self.create_publisher(Image, "~/heightmap/color", 10)
        self.pub_height = self.create_publisher(Image, "~/heightmap/height", 10)
        self.pub_height_vis = self.create_publisher(Image, "~/heightmap/height_vis", 10)
        self.pub_q = self.create_publisher(Image, "~/heightmap/q_canvas", 10)
        self.pub_pose = self.create_publisher(PoseStamped, "~/heightmap/grasp_pose", 10)

        # ---- model ----
        use_cuda = torch.cuda.is_available() and not self.get_parameter("force_cpu").value
        ...
        self.create_subscription(
            PointCloud2, self.pcd_topic, self._on_pcd, qos_profile_sensor_data
        )

        self.get_logger().info("Grasp inference node started (single rotation, angle=0°)")

    # ---------------- callback ----------------

    @torch.inference_mode()
    def _on_pcd(self, msg: PointCloud2):
        H, W = msg.height, msg.width
        if H <= 1:
            return
        ...
        dtype = np.dtype({
            ...
            "offsets": [fields[n].offset for n in ("x","y","z","rgb")],
            "itemsize": msg.point_step,
        })
        arr = np.frombuffer(msg.data, dtype=dtype, count=H*W)

        xyz = np.stack([arr["x"], arr["y"], arr["z"]], -1).reshape(H, W, 3)
        rgb = unpack_rgb_pcl_float(arr["rgb"]).reshape(H, W, 3)

        # ---- build heightmaps ----
        color_hm, height_hm, _ = build_heightmaps(rgb, xyz, self.spec)

        stamp = msg.header.stamp
        frame = self.target_frame or msg.header.frame_id

        self.pub_color.publish(self.bridge.cv2_to_imgmsg(color_hm, "rgb8"))
        self.pub_height.publish(self.bridge.cv2_to_imgmsg(height_hm.astype(np.float32), "32FC1"))

        # height_vis
        vis = np.zeros_like(height_hm, np.uint8)
        m = np.isfinite(height_hm)
        if m.any():
            a, b = np.nanmin(height_hm[m]), np.nanmax(height_hm[m])
            if b > a:
                vis = ((height_hm-a)/(b-a)*255).astype(np.uint8)
        self.pub_height_vis.publish(self.bridge.cv2_to_imgmsg(vis, "mono8"))

        # ---- model forward (single rotation) ----
        ...
        depth_t = torch.nan_to_num(depth_t)
        ...
        print("LOG")
        print(color_t.shape,depth_t.shape)
        ...
        y = self.spec.plane_min[0] + (self.spec.size-1-col)*self.spec.resolution
        z = self.spec.plane_min[1] + (self.spec.size-1-row)*self.spec.resolution
        ...
        pose.pose.orientation.w = 1.0  # угол = 0
        self.pub_pose.publish(pose)

        # ---- q_canvas (single heatmap) ----
        q_norm = q_np - q_np.min()
        if q_norm.max() > 0:
            q_norm /= q_norm.max()

        heat = cv2.applyColorMap((q_norm*255).astype(np.uint8), cv2.COLORMAP_JET)
        heat = cv2.circle(heat, (col,row), 6, (0,0,255), 2)
        self.pub_q.publish(self.bridge.cv2_to_imgmsg(heat, "bgr8"))
